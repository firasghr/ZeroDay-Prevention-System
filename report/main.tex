...
%%  Supervisors: Ghazi Oueslati, Rihab Bousaada
%%  South Mediterranean University Medtech — 2025-2026
%%  Industry Partner: Tunisie Telecom
%%  Compile with:  pdflatex main.tex  (twice for ToC/refs)
%% ============================================================
...
\definecolor{midblue}{RGB}{0,82,153}
...
\definecolor{tikzbox}{RGB}{230,240,255}
\definecolor{tikzarrow}{RGB}{0,82,153}

%% ── TikZ (architecture & lifecycle diagrams) ─────────────────
\usepackage{tikz}
\usetikzlibrary{shapes.geometric,arrows.meta,positioning,fit,shadows.blur,calc,backgrounds,decorations.pathreplacing}

\tikzset{
  block/.style={
    rectangle, rounded corners=4pt,
    draw=deepblue, fill=tikzbox,
    text width=3.2cm, align=center,
    minimum height=1.1cm,
    font=\small\bfseries,
    drop shadow={shadow blur steps=4, shadow xshift=1pt, shadow yshift=-1pt}
  },
  wideblock/.style={block, text width=4.6cm},
  smallblock/.style={block, text width=2.4cm, minimum height=0.9cm, font=\footnotesize\bfseries},
  arrowstyle/.style={-{Stealth[length=6pt]}, thick, color=tikzarrow},
  dasharrow/.style={-{Stealth[length=6pt]}, dashed, thick, color=midblue!60},
  labelbox/.style={rectangle, rounded corners=2pt, fill=deepblue, text=white,
...
\fancyfoot[C]{\thepage}
\fancyfoot[R]{\small\textcolor{gray}{SMU Medtech \textbar{} 2025--2026}}
...
%% ── Captions ─────────────────────────────────────────────────
\usepackage{caption}
\captionsetup{font=small, labelfont={bf,color=deepblue}, margin=10pt, skip=8pt}
...
\titleformat{\chapter}[display]
  ...
  [\vspace{4pt}\titlerule[0.8pt]\vspace{2pt}]
...
\begin{titlepage}
  ...
  \begin{tcolorbox}[
    ...
    \centering
    {\color{white}\large\bfseries South Mediterranean University Medtech}\\[4pt]
    {\color{white}\normalsize Faculty of Computer Science \& Engineering}\\[2pt]
    {\color{white}\small Department of Cybersecurity}
  \end{tcolorbox}

  \vspace{1.5cm}

  %% ── Degree label ───────────────────────────────────────────
  {\normalsize\scshape\color{midblue}
    Capstone Project Report --- Licence in Computer Science}

  \vspace{0.6cm}
  \textcolor{deepblue!40}{\rule{0.6\linewidth}{0.5pt}}
  \vspace{0.6cm}
  ...
  \begin{tcolorbox}[
    ...
    drop shadow={shadow blur steps=5, shadow xshift=2pt, shadow yshift=-2pt,
                 fill=deepblue!30}
  ]
    \centering
  ...
  %% ── Author / supervisor / partner info ─────────────────────
  \begin{tcolorbox}[
    enhanced,
    colback=sectiongray,
    ...
    arc=4pt,
    boxrule=0.8pt,
    left=24pt, right=24pt, top=12pt, bottom=12pt,
  ...
  \begin{tcolorbox}[
    ...
    \centering
    {\color{white}\small
      Capstone Project \textbar{} Cybersecurity Engineering
      \textbar{} SMU Medtech \textbar{} 2025--2026}
  \end{tcolorbox}
  \restoregeometry
\end{titlepage}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[node distance=0.6cm and 1.0cm]
    %% Phase nodes
    \node[smallblock, fill=red!15, draw=red!60]
      (disc) {Discovery\\(Unknown)};
    \node[smallblock, fill=red!25, draw=red!70, right=of disc]
      (priv) {Private\\Exploitation};
    \node[smallblock, fill=orange!20, draw=orange!70, right=of priv]
      (disc2) {Public\\Disclosure};
    \node[smallblock, fill=yellow!20, draw=yellow!70, right=of disc2]
      (patch) {Patch\\Released};
    \node[smallblock, fill=green!20, draw=alertgreen!70, right=of patch]
      (sig) {Signature\\Updated};

    %% Arrows
    \draw[arrowstyle] (disc) -- (priv);
    \draw[arrowstyle] (priv) -- (disc2);
    \draw[arrowstyle] (disc2) -- (patch);
    \draw[arrowstyle] (patch) -- (sig);

    %% Zero-day window brace label
    \draw[thick, decorate, decoration={brace,amplitude=6pt},
          color=alertred]
      ([yshift=8pt]disc.north west) -- ([yshift=8pt]disc2.north east)
      node[midway, above=8pt, font=\scriptsize\bfseries, color=alertred]
           {Zero-Day Window (hours to months)};

    %% AV blind label
    \node[below=0.5cm of priv, font=\scriptsize\itshape, color=alertred,
          align=center]
      {AV/signature-based\\defences: \textbf{blind}};

    %% Protected label
    \node[below=0.5cm of sig, font=\scriptsize\itshape, color=alertgreen,
          align=center]
      {AV effective\\from here};
  \end{tikzpicture}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[node distance=0.7cm and 1.2cm]
    %% Entry point
    \node[wideblock] (main) {\texttt{main.py}\\Entry Point};

    %% Four monitors (below, spread horizontally)
    \node[block, below left=1.0cm and 2.6cm of main]  (pm)  {Process\\Monitor};
    \node[block, below left=1.0cm and 0.6cm of main]  (fm)  {File\\Monitor};
    \node[block, below right=1.0cm and 0.6cm of main] (nm)  {Network\\Monitor};
    \node[block, below right=1.0cm and 2.6cm of main] (dash){Dashboard\\(Flask)};

    %% Detection engine
    \node[wideblock, fill=deepblue!15, draw=deepblue,
          below=2.0cm of pm, xshift=2.2cm] (det) {Detection Engine};

    %% Prevention module
    \node[wideblock, fill=alertred!10, draw=alertred!60,
          below=0.9cm of det] (prev) {Prevention Module};

    %% Storage
    \node[block, fill=green!10, draw=alertgreen!60,
          below=0.9cm of prev] (log)  {\texttt{logs/alerts.json}};

    %% main -> monitors
    \foreach \n in {pm,fm,nm,dash}
      \draw[arrowstyle] (main) -- (\n);

    %% monitors -> detection engine (pm and fm)
    \draw[arrowstyle] (pm) -- (det);
    \draw[arrowstyle] (fm) -- (det);

    %% detection -> prevention
    \draw[arrowstyle] (det) -- (prev);

    %% prevention -> log
    \draw[arrowstyle] (prev) -- (log);

    %% dashboard reads log (dashed feedback)
    \draw[dasharrow] (log.east) -- ++(1.4,0) |-
          node[near end, right, font=\scriptsize, color=midblue]
               {REST API} (dash.east);

    %% network -> detection (dashed, lower priority)
    \draw[dasharrow] (nm) -- ++(0,-0.6) -| (det.north);

    %% Labels
    \node[labelbox, above=4pt of pm]  {\scriptsize Thread 1};
    \node[labelbox, above=4pt of fm]  {\scriptsize Thread 2};
    \node[labelbox, above=4pt of nm]  {\scriptsize Thread 3};
    \node[labelbox, above=4pt of dash]{\scriptsize Thread 4};
  \end{tikzpicture}
...
\begin{figure}[H]
  \centering
  \begin{tcolorbox}[
    enhanced, colback=sectiongray, colframe=deepblue!40,
    arc=4pt, boxrule=0.8pt,
    left=10pt, right=10pt, top=18pt, bottom=18pt,
    width=0.92\linewidth
  ]
    \centering
    \textcolor{deepblue!60}{\large\itshape [Dashboard Screenshot]}\\[6pt]
    \small\color{gray}
    High-risk alert row highlighted in red \textbar{}
    Threat score badge \textbar{} Auto-prevention status indicator
  \end{tcolorbox}
...
\begin{figure}[H]
  ...
  \begin{tcolorbox}[
    enhanced, colback=sectiongray, colframe=deepblue!40,
    arc=4pt, boxrule=0.8pt,
    left=10pt, right=10pt, top=24pt, bottom=24pt,
    width=0.92\linewidth
  ]
    \centering
    \textcolor{deepblue!60}{\large\itshape [Full Dashboard Screenshot]}\\[6pt]
    \small\color{gray}
    Alert table \textbar{} Colour-coded threat badges (High / Medium / Low)
    \textbar{} Search box \textbar{} Numeric threat score column
  \end{tcolorbox}
...
\begin{figure}[H]
  ...
  \begin{tcolorbox}[
    ...
    \centering
    \textcolor{deepblue!60}{\large\itshape [Statistics Panel Screenshot]}\\[6pt]
    \small\color{gray}
    Total alerts counter \textbar{}
    High / Medium / Low breakdown \textbar{}
    Last detection timestamp \textbar{}
    Auto-prevention toggle status
  \end{tcolorbox}
